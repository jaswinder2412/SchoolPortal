...
/* @var $searchModel backend\models\AddClass_search */
		...
		<div class="col-xs-12 index_rowing">
          <div class="box">
            <div class="box-header">
			  ...
			  <?= Html::a('Add', ['add-class/create'], ['class' => 'btn btn-info pull-right']) ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
		...
		<?= GridView::widget([
			...
			'buttons'  => [
				...
				'leadUpdate' => function ($url, $model) {
						...
						$url = Url::to(['add-class/update', 'id' => $model->id]);
				...
				if(Yii::$app->user->identity->role_id == '1'){
					$mikler = ClassInfo::find()->where(['=','class_name',$model->id])->andwhere(['=','school_id',Yii::$app->user->identity->id])->count();
				}else{
					$mikler = ClassInfo::find()->where(['=','class_name',$model->id])->andwhere(['=','school_id',Yii::$app->user->identity->school_id])->count();
				}
					if($mikler > 0){
						$hti = '';
				}else {
					
					 $url = Url::to(['add-class/delete', 'id' => $model->id]);
