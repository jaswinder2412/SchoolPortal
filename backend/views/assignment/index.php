...
<div class="col-xs-12 index_rowing">
          <div class="box box-primary">
            <div class="box-header with-border">
              <h3 class="box-title"> Important Assignments</h3>
            ...
            <div class="box-body">
               <table id="example1" class="table condenser_secodn table-striped table-bordered ">
				    ...
				    <tbody> 
					<?php $getassigned = Assignmentclasses::find()->where(['=','school_id',Yii::$app->user->identity->school_id])->andwhere(['=','Academic_year_id',$presnt_acad])->andwhere(['=','status','1'])->orderBy(['id' => SORT_DESC])->all();  
					$i= 1 ;
					...
					?>
						<tr>
							...
							<td>
								<?php  $stat = "<b>".ucfirst($assgnmntd->anouncement_title)."</b>";
	...
	if(Yii::$app->user->identity->role_id == 1) {
		$geting = Assignmentclasses::find()->where(['=', 'school_id', Yii::$app->user->getId()])->all();
		}else {
			$geting = Assignmentclasses::find()->where(['=', 'school_id', Yii::$app->user->identity->school_id])->all();
		}
      ...
      <div class="modal-content">
		...
		<button type="button" class="close wintagr" data-dismiss="modal"></button> 
			...
			<div class="row">
				...
				<?php if(!empty($getigd->fileupload)){ ?>
			<a href="<?php echo Yii::$app->homeUrl .'upload_file/'.$getigd->fileupload; ?>" download>Click here to download file.</a>
				<?php } ?>
				</div>
...
<?php } else { 
	...
	if(isset($_GET['Assignmentclasses_search']['Academic_year_id'])){
	
		$keysacad = $_GET['Assignmentclasses_search']['Academic_year_id'];
...
?>
 <div class="row pull-right" style="margin-right: 0px; margin-bottom : 7px;">
	<div class="col-md-12">
	
		<span class="margileftplus">Acedemic Year :</span>
		<span>
			<select name="newacademic" id="acadhochngeers">
          ...
          <div class="box">
            <div class="box-header">
              <h3 class="box-title"></h3>
			   <?= Html::a('Create', ['assignment/create'], ['class' => 'btn btn-info pull-right']) ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
			...
			[
			  ...
			  'label' => 'Title',
			   'value' => function ($model) {
				   $stat = "<b>".$model->anouncement_title ."</b>";
				  $gudghs = substr($model->anouncement_description, 0, 70); 
				   $stat.= "<br/>".strip_tags($gudghs)."..."; 
				   return $stat;
			   },
			],
			   ...
			   'value' => function ($model) {
				   ...
				   if(in_array('0',$xclusion)){
					   ...
					   foreach($xclusion as $mrge_class){
						...
						$getings[] = $getclass['class_name']."<br/> ";   
					   }
			...
			'buttons'  => [
				...
				'leadUpdate' => function ($url, $model) {
					...
					if($model->created_by == Yii::$app->user->identity->id){
						$url = Url::to(['assignment/update', 'id' => $model->id]);
				...
				'leadDelete' => function ($url, $model) {
					if(Yii::$app->user->identity->role_id == 5){
					if($model->created_by == Yii::$app->user->identity->id){
						$url = Url::to(['assignment/delete', 'id' => $model->id]);
  ...
  'searching': false,
   pageLength: 20,
    ...
    drawCallback: function(settings){   
	 ...
	 'language': {
      'emptyTable': 'No Assignment available.'
    }
}  );
...
$('#acadhochngeers').change(function(){
	var mkoltire = $(this).val();
	var mkirtes = '/assignment/index?Assignmentclasses_search[Academic_year_id]='+mkoltire;
	window.location.replace(mkirtes);
});
