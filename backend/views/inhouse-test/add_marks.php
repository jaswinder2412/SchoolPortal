...
<?php
	...
	$getexamclassnamesm = InhouseTest::find()->where(['id'=>$id])->andwhere(['=','academic_year',$getacadid])->one();
...
<div class="succeed_messages">
	...
	<i class="icon fa fa-check"></i><?php echo "Marks"; ?> has been Updated.
...
<div class="row ">
		<div class="col-xs-12 index_rowing">
          <div class="box">
			 ...
			 $mitzersd =  AddTest::find()->where(['id'=>$getexamclassnamesm['exam_id']])->one();
				
					$nersect = Section::find()->where(['id'=>$section_id])->one();
			  
				echo "<span style='font-size:16px;'><b>Test Name :</b> ".ucfirst($mitzersd['test_name'])." <span style='margin-left: 12px;'><b>Class:</b> ".  ucfirst($adgetd['class_name'])." ".ucfirst($nersect['section_name'])."</span></span>"; ?></h3>
            </div> 
			...
			<div class="box-body">
				...
				<thead>
				  ...
				  $getsubjctss = ClassSubject::find()->where(['class_id'=>$main_class->id])->andwhere(['Assigned_teacher_id'=>Yii::$app->user->getId()])->all();
					   ...
					   $getsubjctss = InhouseTestSubjects::find()->where(['inhouse_test_id'=>$exam_id])->all();
				  }
				  $miklod = 0;
				  foreach($getsubjctss as $subjectings){
					  
					  if(Yii::$app->user->identity->role_id == 5){
								$finlsumjects = InhouseTestSubjects::find()->where(['inhouse_test_id'=>$exam_id])->andwhere(['Assigned_teacher_id'=>Yii::$app->user->getId()])->andwhere(['subject_id'=>$subjectings->subject_id]);
				  ...
				  echo "<th>".$subject_name['subject_name'] ."<p class='text_one_new'>".$max_marks."</p></th>";
					  $miklod++; }
					  }
				...
				?>
					...
					echo "<div class='mytree'>".Html::img(Yii::getAlias('@base').'/uploads/'.$hdsd,['width' => '70px','class' => 'bord_img'])  
					  ...
					  if($getsubjctsscounts > 0){
							$getval = InhouseTestGrading::find()->where(['inhouse_test_id'=>$exam_id])->andwhere(['class_id'=>$class_id])->andwhere(['section_id'=>$section_id])->andwhere(['subject_id'=>$subjectings->subject_id])->andwhere(['student_id'=>$studentlisting['student_id']])->one();
						...
						<td>
							...
							<input type="hidden" class="maxmarks_hidden" id="max_markss_iid_<?php echo $subjectings->id; ?>" Value="<?php echo $milokeronem['maximum_marks']; ?>"> 
...
$this->registerJsFile('/js/edit_inhouse_marks.js', ['position' => yii\web\View::POS_END, 'depends' => ['yii\web\JqueryAsset']]);
 ?>
</div>