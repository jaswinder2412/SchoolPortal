...
<?php
	...
	$getmysubejct = array();
	$gtsubjctsl = ClassSubject::find()->where(['Assigned_teacher_id'=>Yii::$app->user->getId()])->andwhere(['=','academic_year',$academic_years['id']])->groupBy(['subject_id'])->all();
              ...
              <div class="box-body">
				...
				<div class="form-group">
					  <?= $form->field($model, 'exam_name',[
                        'template' => '<label> Test Name </label><div class="input-group"><div class="input-group-addon"><i class="fa fa-graduation-cap"></i></div>{input}</div>{hint}{error}'
                    ])->textInput() ?>
				</div>
				<div class="form-group">
					  <?= $form->field($model, 'exam_date',[
                        'template' => '<label> Test Date </label><div class="input-group"><div class="input-group-addon"><i class="fa fa-calender"></i></div>{input}</div>{hint}{error}'
                    ])->textInput() ?>
				</div>
