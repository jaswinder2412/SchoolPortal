		...
		<div class="col-xs-12 index_rowing">
          <div class="box">
            <div class="box-header">
			  ...
			  <?= Html::a('Add', ['add-grades/create'], ['class' => 'btn btn-info pull-right']) ?>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
		...
		<?= GridView::widget([
			...
			'columns' => [
			  ...
			  'label' => 'Grade Name',
			   'value' => function ($model) {
				   $rightd = ucfirst($model->grade_name);		
					return $rightd;
			   }
				],
				[
			...
			'buttons'  => [
				...
				'leadUpdate' => function ($url, $model) {
						...
						$url = Url::to(['add-grades/update', 'id' => $model->id]);
				...
				if(Yii::$app->user->identity->role_id == '1'){
					$mikler = FinalExamGrading::find()->where(['=','grade',$model->grade_name])->count();
				}else{
					$mikler = FinalExamGrading::find()->where(['=','grade',$model->grade_name])->count();
				}
					if($mikler > 0){
					 ...
					 $url = Url::to(['add-grades/delete', 'id' => $model->id]);
