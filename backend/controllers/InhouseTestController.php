...
use backend\models\InhouseTest_search;
...
/**
 * InhouseTestController implements the CRUD actions for InhouseTest model.
 */
class InhouseTestController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    { 
		 ...
		 if(isset(Yii::$app->user->identity->id)){
			if (Yii::$app->user->identity->role_id == 5){
			 $arr1=array('index','update','delete','view','create','getsection','addsubject','update_markes','update_gradess','add_marks');
		 }else {
			 $arr1=array('delete');
		 }
		 }else{
    ...
    /**
     * Lists all InhouseTest models.
     * @return mixed
     */
      public function actionIndex()
    {
		$model = new InhouseTest();
        $model_second = new InhouseTestSubjects();
        $searchModel = new InhouseTest_search();
		...
		if(Yii::$app->request->isPost && isset(Yii::$app->request->post()['InhouseTest']['section_id'])){
			$mitzrd = Yii::$app->request->post()['InhouseTest']['section_id'];
			$eexam_id = Yii::$app->request->post()['InhouseTest']['exam_id'];
			$classss_id = Yii::$app->request->post()['InhouseTest']['class_id'];
			foreach($mitzrd as $subjectsda){
				if(!empty($subjectsda)){
					...
					$Examinations_count = InhouseTest::find()->where(['exam_id'=>$eexam_id])->andwhere(['class_id'=>$classss_id])->andwhere(['section_id'=>$subjectsda])->andwhere(['school_id'=>$schol_ild])->andwhere(['academic_year'=>$acsdmif])->count();
				...
				/* 	if($Examinations_count == 0){ */
						...
						if($getclass_subjects_counting > 0){
							...
							$model->teacher_created_by = Yii::$app->user->getId();
							$model->section_id = $subjectsda;
							$model->academic_year = $saddfs['id'];
							$model->created = time();
							$model->updated = time();
							$model->save(false);
							Yii::$app->session->setFlash('success', "Test has been Added.");
				...
				/* 	else
					{
						$clks_nmd = AddClass::find()->where(['id'=>$classss_id])->one();
						$fgsd_sction = Section::find()->where(['id'=>$subjectsda])->one();
						$fgsd_examinas = AddTest::find()->where(['id'=>$eexam_id])->one();
						$war_msg = "Test ".$fgsd_examinas['test_name']." and Class ".$clks_nmd['class_name']. " and Section ".$fgsd_sction['section_name']." Already exist.";
						Yii::$app->session->setFlash('error', $war_msg);
					} */
				}
    ...
    /**
     * Displays a single InhouseTest model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new InhouseTest model.
    ...
    /**
     * Updates an existing InhouseTest model.
    ...
    /**
     * Deletes an existing InhouseTest model.
    ...
    public function actionDelete($id)
    {
		...
		$second_mosdel = InhouseTestSubjects::find()->where(['=','inhouse_test_id',$id])->all();
	...
	public function actionGetsection()
    {
	   ...
	   $academine = AcedemicYear::find()->Where(['=', 'school_id', Yii::$app->user->identity->school_id])->AndWhere(['=', 'status', 1])->one();
		...
		foreach($getingsection as $sectionwise) {
			...
			$getsureforsubject = ClassSubject::find()->where(['class_id'=>$sectionwise->id])->andwhere(['Assigned_teacher_id'=>Yii::$app->user->getId()])->count();
			if($getsureforsubject > 0){
				$getsections_name = Section::find()->where(['id'=>$sectionwise->class_section])->one();
			echo "<span class='mrtzs'><input type='checkbox' name='InhouseTest[section_id][]' id='InhouseTest-section_id' value='".$getsections_name['id']."'> ".$getsections_name['section_name']."</span>";
			}
	...
	public function actionAddsubject()
	{
		...
		$final_ids = $exam_id;
				$getcount = InhouseTestSubjects::find()->where(['=','inhouse_test_id',$final_ids])->count();
				if($getcount > 0){
					$second_mosdel = InhouseTestSubjects::find()->where(['=','inhouse_test_id',$final_ids])->all();
					foreach($second_mosdel as $deltmodal) {
						$deltmodal->delete();
					}
				...
				$model_second->load(Yii::$app->request->post());
					...
					$model_second->inhouse_test_id = $final_ids;
	...
	public function actionUpdate_gradess()
    {
		...
		foreach($subjects_id as $key => $subject_id){
			$getval = InhouseTestGrading::find()->where(['inhouse_test_id'=>$exam_id])->andwhere(['class_id'=>$class_id])->andwhere(['subject_id'=>$subject_id])->andwhere(['student_id'=>$student_id])->one();
		if(!empty($getval)){
			$getval['inhouse_test_id'] = $exam_id;
			...
			$model = new InhouseTestGrading();
			$model->load(Yii::$app->request->post());
			$model->inhouse_test_id = $exam_id;
	...
	public function actionUpdate_markes()
    {
		...
		foreach($subjects_id as $key => $subject_id){
			$getval = InhouseTestGrading::find()->where(['inhouse_test_id'=>$exam_id])->andwhere(['class_id'=>$class_id])->andwhere(['subject_id'=>$subject_id])->andwhere(['teacher_created_by'=>Yii::$app->user->getId()])->andwhere(['student_id'=>$student_id])->one();
		if(!empty($getval)){ 
			...
			$getval['teacher_created_by'] = Yii::$app->user->getId();
			$getval->save(false);
		}else{
    ...
    /**
     * Finds the InhouseTest model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return InhouseTest the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = InhouseTest::findOne($id)) !== null) {
            return $model;
        } else {
