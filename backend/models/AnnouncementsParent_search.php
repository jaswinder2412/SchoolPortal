...
/**
 * AnnouncementsParent_search represents the model behind the search form about `backend\models\AnnouncementsParent`.
 */
class AnnouncementsParent_search extends AnnouncementsParent
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['id'], 'integer'],
            [['class_id', 'section', 'Academic_year_id', 'school_id', 'student_id', 'anouncement_title', 'anouncement_description', 'status', 'created', 'updated'], 'safe'],
        ];
    }
    ...
    {
        $query = AnnouncementsParent::find();
		if(Yii::$app->user->identity->role_id == 1) {
			$query->andFilterWhere(['=', 'school_id', Yii::$app->user->getId()])->andFilterWhere(['=', 'created_by', Yii::$app->user->getId()]);
		}else {
